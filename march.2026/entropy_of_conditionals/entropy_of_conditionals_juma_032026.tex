...
\begin{equation}
    \beta_{J}(y,x,Y) = {\lceil log_{2}(\#Y) \rceil} - log_{2}(P(y|x)^{-1}) = log_{2}({\lceil \#Y \rceil}P(y|x))
\end{equation}
...
While this is ultimately not a necessarily unique formulation, in the wider context of application to conditional observations and antecedent entropy it poses a useful metric by which to engineer systems such as statistical learning methods.
